...
轻量易用的存储管理器，统一浏览和管理本地目录与 S3 兼容对象存储。支持文件预览、内容搜索、批量操作、跨存储传输和 S3 对象管理，无需部署 Filo 后端服务。连接信息保存在本机，添加存储空间不会导入或搬移原有文件。
...
侧栏位置的右键菜单支持编辑、移除连接。本地连接可修改名称、只读状态和根目录；S3 连接可修改访问配置及凭据。移除位置保留原始文件或对象；该位置有未完成传输时，需先取消或等待任务完成。
...
- 默认轮询外部目录变化，本地每 5 秒、S3 每 15 秒，可关闭自动刷新；读取失败后停止轮询，支持手动重试。
- 详情面板显示文件属性；双击本地普通文件用系统默认应用打开，S3 文件可在应用内预览或下载。

界面固定为浅色，提供存储概览、文件浏览、传输任务和设置页面。常用操作可通过工具栏、右键菜单、「更多操作」及桌面系统「操作」菜单访问；未处理异常提供诊断信息查看与复制。

### 文件操作与传输

- 本地与 S3 均支持新建文件夹、重命名、复制、移动、删除；可在已连接的本地 / S3 位置之间批量传输文件和文件夹，保留目录结构。
- 从 Finder / 文件资源管理器拖入文件或文件夹，复制到当前可写目录；S3 工具栏还支持通过原生选择器批量上传文件、下载单个文件。
- 复制、移动、重命名和上传支持同名冲突策略：拒绝、覆盖、跳过、自动改名。同名文件夹覆盖时合并内容，保留目标独有项目。
...
普通删除在后端支持时优先移入回收站；回收站失败不会自动永久删除，需另行确认。不支持回收站的 S3 后端会明确提示永久删除。「永久删除」跳过回收站并单独确认，文件夹连同全部内容删除。
...
使用与实现细节见 [S3 与本机 RustFS](docs/11-s3.md)、[批量与文件夹操作](docs/12-batch-and-folders.md)、[冲突处理与大目录](docs/13-conflicts-and-paging.md)、[传输速度设置](docs/13-transfer-speed.md) 和 [浏览与 S3 高级管理](docs/14-browsing-and-s3-management.md)。[开发指导索引](docs/README.md) 同时保留原始设计与阶段实施记录，当前功能以代码和本 README 为准。
...
`provider-opendal` 的 `local/` 与 `s3/` 分别封装适配器，`s3_admin` 和 `tos_stats` 提供对象管理与 TOS 桶统计。`storage-application` 按存储位置管理（`volumes`）、文件操作（`entries`）、传输调度与执行（`transfers`）、预览与搜索（`browsing`）等职责划分；`storage-provider-api` 提供存储接口与共享限速器。
...
macOS 配置数据库位于 `~/Library/Application Support/dev.filo.desktop/filo.sqlite`，保存连接、传输记录和限速设置；S3 密钥不写入 SQLite。当前浏览偏好只在本次会话保留。
