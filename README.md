...
- 详情面板显示文件属性；双击本地普通文件用系统默认应用打s's开，远程文件可在应用内预览或下载。

界面固定为浅色，提供存储概览、文件浏览、传输任务和设置页面。常用操作可通过工具栏、右键菜单、「更多操作」及桌面系统「操作」菜单访问；未处理异常提供诊断信息查看与复制。

### 文件操作与传输

- 本地、S3、FTP / FTPS、SFTP 和 SMB 支持新建文件夹、上传、重命名、复制、移动、删除；跨存储批量传输保留目录结构。
...
- 复制、移动、重命名和上传支持同名冲突策略：拒绝、覆盖、跳过、自动改名。同名文件夹覆盖时合并内容，保留目标独有项目。远程覆盖先校验原文件再替换；具体边界见[远程存储说明](docs/15-remote-storage-and-file-workflow.md)。
...
`provider-opendal` 的 `local/` 与 `s3/` 分别封装适配器，`remote/` 封装 FTP、SFTP 和 SMB 协议，`s3_admin` 和 `tos_stats` 提供对象管理与 TOS 桶统计。`storage-application` 按存储位置管理（`volumes`）、文件操作（`entries`）、传输调度与执行（`transfers`）、预览与搜索（`browsing`）等职责划分；`storage-provider-api` 提供存储接口与共享限速器。
